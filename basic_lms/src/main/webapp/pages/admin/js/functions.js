...
// 상단 right 내 요소 통합 반응형
function col_return(){
    if($(window).width() <= 1200){
        $('.basic_top_searchbox .right').css({'display':'flex', 'width':'100%', 'flex-wrap':'wrap'});
        // $('.basic_top_searchbox .right > .col').addClass('s6 m4 l4');
        $('.basic_top_searchbox .right > .col').css({'flex':'0 1 120px'});
        $('.basic_top_searchbox .right button').css({'width':'100%'});
        $('.basic_top_searchbox .right a').css({'width':'100%'});

        if($(window).width() <= 600){
            $('.basic_top_searchbox .right > .col').css({'flex':'1 1 50%'});

            if($(window).width() <= 400){
                $('.basic_top_searchbox .right > .col').css({'flex':'1 1 100%'});
                // 종료 - 조건부에 따름
                return;
            }
            // 종료 - 조건부에 따름
    ...
    $('.basic_top_searchbox .right').css({'display':'block', 'width':'auto'});
    $('.basic_top_searchbox .right > .col').removeClass('s6 m4 l4');
    $('.basic_top_searchbox .right button').css({'width':'initial'});
    $('.basic_top_searchbox .right a').css({'width':'initial'});
}
// 크기 변화 대응
...
function user_guide_flow() {
    ...
    // 가이드 활성화
    if (guide_switch && !getCookie('i_guide')) {        // 임시적 쿠키로 적용함 > DB로 적용할 것을 권장
        // 클래스 추가(하이라이트 요소)
    ...
    function clone_highlighting(f_info){
        ...
        if (clone_style_info_bg === 'rgba(0, 0, 0, 0)' || clone_style_info_bg === 'rgba(255, 255, 255, 0)') {
        modifiedBgColor = 'rgb(255, 255, 255)';
        }
